...
- active job stop 會透過 worker per-job cancel flag 影響 join、lobby wait 與 recording monitor；provider lobby wait 必須接收 cancel callback，避免 `waiting_lobby` job 卡到 `lobby_wait_sec` timeout 才收斂
- 每個 active recording attempt 會取得一個 `RuntimeResourceLease`，包含獨立 Xvfb display 與 Pulse/PipeWire sink，例如 `:100`、`mr_sink_<job_id>`
- Webex 錄製會額外把 per-job sink monitor 以 Pulse/PipeWire `module-remap-source` 暴露成瀏覽器可見的 per-job microphone source；FFmpeg 仍只錄 `pulse_monitor`，這個 browser source 只用來通過 Webex prejoin media-device gate
...
- `RecordingSession.prepare_capture_surface()` 會在 FFmpeg 前 best-effort 送出 Escape，關閉 Chrome 外部協定提示等 browser chrome modal；這不是 provider DOM overlay，避免 Webex `webex://` / `xdg-open` prompt 被錄進畫面
...
- 若 app mode 在 FFmpeg 開始前發生 browser/app-window 技術失敗，worker 會用同一 logical result 進行一次 normal mode attempt；若原 crop mode 是 `off`，fallback 會改用 `auto` 以避免錄到 browser chrome。
- provider join/admit 階段已回報 `JOIN_TIMEOUT`、`LOBBY_TIMEOUT`、`NEVER_JOINED` 或 provider-specific error code 時，視為會議加入結果，不再觸發 normal browser fallback。
...
Telegram recording stage、completion、failure、retry 與 upload notifications 都是 best-effort；send/edit/fallback-send 會以最多 3 chat 的 bounded concurrent fanout 執行，且每個 Telegram API 呼叫都有 10 秒 timeout。Timeout 或 API exception 只記錄 log，不能阻斷錄製、後處理或上傳主流程；Telegram 回報 `Message is not modified` 時視為 edit no-op success，不可 fallback-send 造成重複訊息。Recording lifecycle notification 的使用者可見內容固定使用英文，標題優先透過 `recording_jobs.schedule_id -> schedules.meeting_id -> meetings.name` 顯示有效 meeting name；空白或含 replacement character 的壞 label 會被略過，接著 fallback 有效且非 generic 的 `display_name`，最後才 fallback `meeting_code`。訊息會顯示 Provider / Status，Started / Ended 使用設定時區的 `YYYY-MM-DD HH:MM`，Duration 使用 minutes。現行 DB 仍只保存單一 `telegram_message_id`，多 chat per-message tracking 留到未來 schema 版本。
